...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({ name: "Demo Operator", email: "demo@optitwin.ai", password: "demo-password", confirm: "demo-password" });
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4">
      <form onSubmit={submit} className="app-surface w-full max-w-md p-6">
        ...
        <p className="mt-2 text-sm text-slate-400">Create a local workspace user and start building scenarios.</p>
        ...
        <input className="field mt-2" type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} />
        <label className="mt-4 block text-sm text-slate-300">Confirm password</label>
        <input className="field mt-2" type="password" value={form.confirm} onChange={(event) => setForm({ ...form, confirm: event.target.value })} />
